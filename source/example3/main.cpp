...
int main(int /*argc*/, char** argv)
{
    ...
    auto* cost_fun = new ceres::NumericDiffCostFunction<BlackBoxCostFunctor, ceres::CENTRAL, 1, 1, 1, 1, 1>(new BlackBoxCostFunctor());
    problem.AddResidualBlock(cost_fun, nullptr, &a1, &l1, &a2, &l2);

    // Linear search does not work with these constrains
    ...
    //~~ No idea what it does, but works (much) better than default~~
    // Default is LBFGS, which is fastest
    ...
    // with err^2
    // ceres::NonlinearConjugateGradientType::FLETCHER_REEVES;  | y = 2.23778*exp(-1.89343*x) + 0.960176*exp(-0.195231*x) | Ceres Solver Report: Iterations: 65, Initial cost: 6.831329e+04, Final cost: 5.431750e-03, Termination: CONVERGENCE
    // ceres::NonlinearConjugateGradientType::POLAK_RIBIERE;    | y = 2.24586*exp(-1.88566*x) + 0.95368*exp(-0.193838*x)  | Ceres Solver Report: Iterations: 66, Initial cost: 6.831329e+04, Final cost: 5.435482e-03, Termination: CONVERGENCE | 
    // ceres::NonlinearConjugateGradientType::HESTENES_STIEFEL; | y = 2.24441*exp(-1.85627*x) + 0.946983*exp(-0.193059*x) | Ceres Solver Report: Iterations: 57, Initial cost: 6.831329e+04, Final cost: 5.467787e-03, Termination: CONVERGENCE
    // ceres::LineSearchDirectionType::LBFGS (default)          | y = 2.23903*exp(-1.89234*x) + 0.959393*exp(-0.195078*x) | Ceres Solver Report: Iterations: 41, Initial cost: 6.831329e+04, Final cost: 5.431627e-03, Termination: CONVERGENCE
    // ceres::LineSearchDirectionType::BFGS                     | y = 2.23906*exp(-1.89238*x) + 0.959396*exp(-0.19508*x)  | Ceres Solver Report: Iterations: 59, Initial cost: 6.831329e+04, Final cost: 5.431628e-03, Termination: CONVERGENCE
    // Example 1                                                | y = 0.959354*exp(-0.195071*x) + 2.23905*exp(-1.89224*x) |
#else
    // These do not work
